...
def build_simple_explanation_prompt(analysis_text: str, product_id: str) -> str:
    product_name = PRODUCT_NAMES.get(product_id, product_id)
    return (
        f"Ты объясняешь инвестиции максимально просто — как школьнику 4 класса, "
        f"используя бытовые аналогии и примеры из реальной жизни. Без финансового жаргона.\n\n"
        f"Перед тобой профессиональный анализ влияния новости на продукт «{product_name}»:\n\n"
        f"{analysis_text}\n\n"
        f"Объясни то же самое простыми словами. Используй этот формат:\n\n"
        f"🧒 <b>Простым языком</b>\n\n"
        f"[2–3 предложения с бытовой аналогией — например через карманные деньги, "
        f"магазин, копилку, огород и т.п.]\n\n"
        f"💡 <b>Вывод одной фразой:</b> [максимально коротко — хорошо/плохо/нейтрально для продукта и почему]\n\n"
        f"Требования: никакого финансового жаргона, максимум 5 предложений суммарно, "
        f"HTML-теги <b> и <i>."
    )
...
def build_news_analysis_prompt(news_text: str, product_id: str) -> str:
    ...
    if product_id == "general":
        client_advice = (
            "💼 <b>Новому клиенту:</b> [1–2 предложения: какой инструмент из линейки Альфа-Банка "
            "наиболее актуален в текущей ситуации и почему]\n\n"
            "📋 <b>Действующему клиенту:</b> [1–2 предложения: общая рекомендация по позиции — "
            "держать, докупать или фиксировать прибыль, с кратким обоснованием]"
        )
    else:
        client_advice = (
            f"💼 <b>Новому клиенту:</b> [1–2 предложения: стоит ли сейчас входить в «{product_name}» "
            f"с учётом этой новости, и если нет — что лучше рассмотреть из линейки Альфа-Банка]\n\n"
            f"📋 <b>Действующему клиенту</b> (уже оформившему «{product_name}»): "
            f"[1–2 предложения: ждать / докупать / выходить — конкретное действие с обоснованием]"
        )
    ...
    return (
        ...
        f"НОВОСТЬ:\n{news_text}\n\n"
        f"Ответь строго в следующем формате (без отступлений, HTML-теги <b> и <i>):\n\n"
        f"📰 <b>Анализ влияния новости</b>\n\n"
        f"📌 <b>Продукт:</b> {product_name}\n"
        f"[📈 Влияние: Позитивное / 📉 Влияние: Негативное / ➡️ Влияние: Нейтральное]\n\n"
        f"[3–5 предложений: механизм влияния на данный продукт, ссылаясь на его состав]\n\n"
        f"{client_advice}\n\n"
        f"⚠️ <i>Не является инвестиционной рекомендацией.</i>\n\n"
        f"Требования: придерживайся формата выше, без повтора полного состава продукта, "
        f"конкретные рекомендации опирай на описанный состав продукта."
    )
